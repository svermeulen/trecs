...
- **Positional-tag attribute forms.** `[ForEachAspect]`, `[ForEachEntity]`, `[SingleEntity]`, and `[FromWorld]` now accept tags both as positional generic arguments — e.g. `[ForEachEntity<EcsTags.Enemy>]` — and as constructor-positional `typeof(...)` arguments — e.g. `[ForEachEntity(typeof(EcsTags.Enemy))]`. The verbose named-property `Tag = typeof(...)` form remains supported. Docs use the new generic-positional form; samples retain the constructor-positional form.
...
- **`[GlobalIndex]`** parameter attribute on iteration `Execute` methods — receives the packed 0..N-1 index across all groups iterated by the call. Useful for writing into a single contiguous `NativeArray` shared across groups. Job-side only; works in both manual job structs and `[WrapAsJob]`-generated jobs.
...
- **"Bookmark" terminology replaced with "Saves" in the editor window.** The old `TrecsBookmarksWindow` is now `TrecsSavesWindow`. The runtime recording API still uses `Bookmark` for individually labelled markers within a `RecordingBundle` (distinct from `Anchor`, which is an auto-placed scrub/recovery point). Migration: rename `TrecsBookmarksWindow` → `TrecsSavesWindow`; runtime call sites (`BundleRecorder.CaptureBookmarkAtCurrentFrame`, `RecordingBundle.Bookmarks`, `BundleBookmark`) are unchanged.
