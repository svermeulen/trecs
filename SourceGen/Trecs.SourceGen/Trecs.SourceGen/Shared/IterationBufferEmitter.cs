...
namespace Trecs.SourceGen.Shared
{
    ...
    /// expects the iteration buffers in the form `(TypeDisplay, ReadOnly)` returned by
    ...
    internal static class IterationBufferEmitter
    {
        ...
        internal static void EmitDepRegistration(
            StringBuilder sb,
            string indent,
            IReadOnlyList<(string TypeDisplay, bool ReadOnly)> buffers
        )
        {
            ...
            for (int i = 0; i < buffers.Count; i++)
            {
                ...
                var rid = $"ResourceId.Component(TypeId<{typeName}>.Value)";
        ...
        /// (`__trecs_buf{i}_value`) via the world's per-job buffer accessors.
        /// </summary>
        ...
        )
        {
            ...
            for (int i = 0; i < buffers.Count; i++)
            {
                ...
                sb.AppendLine(
                    $"{indent}{p}scheduler.{method}({p}handle, {rid}, {p}group, {p}jobName);"
                );
            }
        }
    }
