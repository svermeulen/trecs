...
namespace Trecs.Samples
{
    public static class SampleUtil
    {
        ...
        public static Material CreateUnlitInstancedMaterial(Color color)
        {
            // Indirect + structured-buffer path needs compute; WebGL 2 and some
            // low-end mobile GPUs fall back to standard GPU instancing.
            var useFallback = SampleRenderingPath.UseFallback;
            var shaderName = useFallback
                ? "Trecs/InstancedUnlitFallback"
                : "Trecs/InstancedIndirectUnlit";
            var material = new Material(Shader.Find(shaderName));
