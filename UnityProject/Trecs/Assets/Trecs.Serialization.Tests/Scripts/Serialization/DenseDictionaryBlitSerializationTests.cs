...
namespace Trecs.Serialization.Tests
{
    ...
    public class DenseDictionaryBlitSerializationTests
    {
        ...
        public void DenseDictionary_EmptyDictionary_SerializesAndDeserializes()
        {
            ...
            Assert.That(deserialized.Count == 0);
        }
        ...
        public void DenseDictionary_IntFloat_SerializesAndDeserializes()
        {
            ...
            // Assert
            Assert.IsNotNull(deserialized);
            Assert.That(deserialized.Count == original.Count);
        ...
        public void DenseDictionary_LargeDictionary_SerializesAndDeserializes()
        {
            ...
            // Assert
            Assert.IsNotNull(deserialized);
            Assert.That(deserialized.Count == count);

            // Check first, middle, and last elements
            Assert.That(deserialized[0] == original[0]);
            Assert.That(deserialized[5000] == original[5000]);
            Assert.That(deserialized[9999] == original[9999]);
        ...
        public void DenseDictionary_SingleElement_SerializesAndDeserializes()
        {
            ...
            // Assert
            Assert.IsNotNull(deserialized);
            Assert.That(deserialized.Count == 1);
            Assert.That(deserialized[42] == 123);
        }
        ...
        public void DenseDictionary_PreInitialized_DeserializesCorrectly()
        {
            ...
            foreach (var kvp in original)
            {
                Assert.That(deserialized[kvp.Key] == kvp.Value);
            }
        }
        ...
        public void DenseDictionary_BlitPerformanceTest_CompareWithRegularDictionary()
        {
            ...
            // Verify correctness
            Assert.That(deserializedSvDict.Count == elementCount);
            Assert.That(deserializedRegularDict.Count == elementCount);
            for (int i = 0; i < elementCount; i++)
            {
                Assert.That(deserializedSvDict[i] == deserializedRegularDict[i]);
            }
