...
- **`ManagedSharedPtr`** — `HeightmapData` is a managed class holding a
  ...
  via `heightmap.Value.Get(World.Heap)`.
...
- **`ManagedSharedPtrInterface`** — `MutableHeightmapData` is a mutable
  ...
  `IReadOnlyHeightmapData` interface (interface adoption path — see
  [Sample 19 — Shared Interface](../19_SharedInterface/README.md)). The
