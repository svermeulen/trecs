...
namespace Trecs.Samples.HeightmapBlobs
{
    ...
    public class SceneInitializer : IDisposable
    {
        ...
        void InitializeInterface(WorldAccessor world, HeightmapDescriptor descriptor, BlobId blobId)
        {
            ...
            // Note that for this sample, cache miss always happens since our blob store
            // is inmemory only
            if (!SharedPtr.TryGet<IReadOnlyHeightmapData>(world.Heap, blobId, out _interfaceAnchor))
            {
                ...
                _interfaceAnchor = SharedPtr.Alloc<IReadOnlyHeightmapData>(
                    world.Heap,
                    blobId,
                    data
                );
        ...
        void SpawnManaged(WorldAccessor world, float offset, float3 initial)
        {
            ...
            world
                ...
                .Set(new ManagedHeightmapRef(_managedAnchor.Clone(world.Heap)))
                .AssertComplete();
        }
        ...
        void SpawnNative(WorldAccessor world, float offset, float3 initial)
        {
            world
                ...
                .Set(new Position(initial))
                .Set(new NoiseOffset(offset))
                .Set(new NativeHeightmapRef(_nativeAnchor.Clone(world.Heap)))
                .AssertComplete();
        }
        ...
        void SpawnNativeLarge(WorldAccessor world, float offset, float3 initial)
        {
            world
                ...
                .Set(new Position(initial))
                .Set(new NoiseOffset(offset))
                .Set(new NativeHeightmapRefLarge(_nativeLargeAnchor.Clone(world.Heap)))
                .AssertComplete();
        }
        ...
        void SpawnInterface(WorldAccessor world, float offset, float3 initial)
        {
            world
                ...
                .Set(new Position(initial))
                .Set(new NoiseOffset(offset))
                .Set(new InterfaceHeightmapRef(_interfaceAnchor.Clone(world.Heap)))
                .AssertComplete();
        }
        ...
        public void Dispose()
        {
            ...
            {
                _managedAnchor.Dispose(world.Heap);
            }
            if (!_nativeAnchor.IsNull)
            {
                _nativeAnchor.Dispose(world.Heap);
            }
            if (!_nativeLargeAnchor.IsNull)
            {
                _nativeLargeAnchor.Dispose(world.Heap);
            }
            if (!_interfaceAnchor.IsNull)
            {
                _interfaceAnchor.Dispose(world.Heap);
            }
