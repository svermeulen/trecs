...
{
    /// <summary>
    /// This just wraps FixedArray256, adds a count, and does bounds checking
    /// </summary>
    public struct FixedList256<T>
        where T : unmanaged
    {
        public FixedArray256<T> Buffer; // Don't access directly
        ...
        public int Count
        {
            ...
            set
            {
                Require.That(value >= 0 && value <= 256);
                _count = value;
            }
        }
        ...
        {
            FixedTypeCommon.Log.Warning("Used object Equals on FixedList256, causing boxing");
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref T GetRef<T>(this ref FixedList256<T> pose, int index)
            where T : unmanaged
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref readonly T Get<T>(this in FixedList256<T> pose, int index)
            where T : unmanaged
        {
            Require.That((int)index >= 0 && (int)index < pose.Count);
        ...
        public static void Add<T>(this ref FixedList256<T> list, T item)
            where T : unmanaged
        {
            Require.That(list.Count < 256, "FixedList256 is full");
