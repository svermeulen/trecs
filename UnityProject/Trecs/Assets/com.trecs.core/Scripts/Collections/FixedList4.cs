...
{
    /// <summary>
    /// A bounded list with compile-time capacity of 4 elements, stored inline.
    /// </summary>
    [NonCopyable]
    public struct FixedList4<T>
        where T : unmanaged
    {
        public FixedArray4<T> Buffer; // Raw storage — prefer the indexer / Mut() for bounds-checked access.
        ...
        public int Count
        {
            ...
            set
            {
                TrecsAssert.That(value >= 0 && value <= 4);
                _count = value;
            }
        }
        ...
        public override readonly int GetHashCode()
        {
            unsafe
            {
                fixed (FixedArray4<T>* bufPtr = &Buffer)
                {
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref T Mut<T>(this ref FixedList4<T> list, int index)
            where T : unmanaged
        {
            TrecsAssert.That(
        ...
        public static void Add<T>(this ref FixedList4<T> list, T item)
            where T : unmanaged
        {
            TrecsAssert.That(list.Count < 4, "FixedList4 is full");
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void RemoveAt<T>(this ref FixedList4<T> list, int index)
            where T : unmanaged
        {
