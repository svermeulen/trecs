...
{
    /// <summary>
    /// This just wraps FixedArray64, adds a count, and does bounds checking
    /// </summary>
    public struct FixedList64<T>
        where T : unmanaged
    {
        public FixedArray64<T> Buffer; // Don't access directly
        ...
        {
            FixedTypeCommon.Log.Warning("Used object Equals on FixedList64, causing boxing");
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref T GetRef<T>(this ref FixedList64<T> pose, int index)
            where T : unmanaged
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ref readonly T Get<T>(this in FixedList64<T> pose, int index)
            where T : unmanaged
        {
            Require.That((int)index >= 0 && (int)index < pose.Count);
