...
namespace Trecs.Internal
{
    ...
    public class TrecsGameStateController : IDisposable
    {
        ...
        {
            TrecsAssert.That(!string.IsNullOrWhiteSpace(name), "Recording name must not be empty");
            if (!_autoRecorder.IsRecording)
            {
        ...
        {
            TrecsAssert.That(!string.IsNullOrWhiteSpace(name));
            var path = GetRecordingPath(name);
            if (!File.Exists(path))
            {
        ...
        {
            TrecsAssert.That(!string.IsNullOrWhiteSpace(oldName));
            TrecsAssert.That(!string.IsNullOrWhiteSpace(newName));
        ...
        {
            TrecsAssert.That(!string.IsNullOrWhiteSpace(name), "Snapshot name must not be empty");
