...
namespace Trecs
{
    public class FrameScopedUniqueHeap
    {
            ...
            where T : class
        {
            ...
            Assert.That(_poolManager.HasPool<T>());

            Assert.That(value.GetType() == typeof(T));
            ...
            where T : class
        {
            ...
            Assert.That(entry.TypeId == TypeIdProvider.GetTypeId<T>());
        ...
        internal void Deserialize(ITrecsSerializationReader reader)
        {
            ...
            for (int i = 0; i < numEntries; i++)
            {
                ...
                Assert.That(TypeIdProvider.GetTypeId(objType) == typeId);
        ...
        internal void RemapFrameOffsets(int frameOffset)
        {
            ...
            {
                var newEntry = new HeapEntry(
                    oldEntry.Value,
                    oldEntry.TypeId,
                    oldEntry.Frame + frameOffset
                );
                _entries.Add(address, newEntry);
            }

            _log.Debug(
                "Remapped {} input heap entries by {} frames",
