...
namespace Trecs.Serialization
{
    ...
    /// and values. Writes the dictionary's internal structure (nodes + values + buckets)
    /// as raw memory, avoiding the per-element framing that
    /// <see cref="IterableDictionarySerializerManaged{TKey,TValue}"/> incurs.
    /// </summary>
    public sealed class IterableDictionarySerializerUnmanaged<TKey, TValue>
        : ISerializer<IterableDictionary<TKey, TValue>>
        where TKey : unmanaged, IEquatable<TKey>
        where TValue : unmanaged
    {
        public IterableDictionarySerializerUnmanaged() { }
