...
{
    /// <summary>
    /// Serializer for <see cref="List{T}"/> with managed/custom elements. Writes
    /// the item count followed by each element via the registered serializer for
    /// <typeparamref name="T"/>. For lists of unmanaged types use
    /// <see cref="ListSerializerUnmanaged{T}"/> instead, which writes the
    /// elements as a single blit and avoids per-element framing.
    /// </summary>
    public sealed class ListSerializerManaged<T> : ISerializer<List<T>>
        where T : class
    {
        public ListSerializerManaged() { }
