...
{
    /// <summary>
    /// Serializer for <see cref="Queue{T}"/> with managed/custom elements.
    /// Writes the item count followed by each element in FIFO order via the
    /// registered serializer for <typeparamref name="T"/>. For queues of
    /// unmanaged types use <see cref="QueueSerializerUnmanaged{T}"/> instead,
    /// which writes the elements as a single blit and avoids per-element
    /// framing.
    /// </summary>
    public sealed class QueueSerializerManaged<T> : ISerializer<Queue<T>>
        where T : class
    {
        public QueueSerializerManaged() { }
