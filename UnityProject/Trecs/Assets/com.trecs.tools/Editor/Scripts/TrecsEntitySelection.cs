...
namespace Trecs.Tools
{
    ...
    /// the user picks an entity in <see cref="TrecsHierarchyWindow"/>. Holds
    /// the stable <see cref="EntityHandle"/> (not <see cref="EntityIndex"/>, which
    /// shifts on structural change) plus a weak ref to the owning
    ...
    /// </summary>
    public class TrecsEntitySelection : ScriptableObject
    {
        [NonSerialized]
    ...
    /// <see cref="Editor.OnEnable"/>, destroyed in
    /// <see cref="Editor.OnDisable"/>: each Inspector instance gets its own
    /// target, so Unity's <c>PropertyHandlerCache</c> entries (keyed on
    ...
    public class TrecsEntitySelectionInspector : Editor
    {
        ...
        // Per-component access badges (one per component row); refreshed each
        // tick from the access tracker.
        readonly Dictionary<Type, Label> _accessBadges = new();
        readonly Dictionary<Type, int> _lastAccessHashByType = new();
        ...
        void Refresh()
        {
            ...
            try
            {
                _accessor ??= world.CreateAccessor("TrecsEntitySelectionInspector");
            }
            catch (Exception e)
            {
        ...
        void RebuildComponentBoxes(TrecsEntitySelection selection, ResolvedTemplate rt)
        {
            ...
            _buffer = ScriptableObject.CreateInstance<TrecsEntityInspectorBuffer>();
        ...
        )
        {
            ...
            // collapses the type for aspect property names).
            if (row.IsUnwrap && row.UnwrapFieldName != null)
            {
                var inner = valueProp.FindPropertyRelative(row.UnwrapFieldName);
                if (inner != null)
                {
        ...
        // Adds a small "↗" jump icon and an "R N · W M" access badge to the
        // right of the component header. Hover the icon → preview the
        // matching component-type row in the hierarchy; click → select it.
        // The badge text is updated each refresh tick from the access tracker.
        VisualElement WrapWithJumpIcon(VisualElement child, World world, Type componentType)
        {
            ...
            _accessBadges[componentType] = badge;
        ...
        {
            var label = new Label($"{row.DisplayName}: {suffix}");
            label.style.opacity = 0.5f;
            parent.Add(WrapWithJumpIcon(label, world, row.ComponentType));
        }

        void RefreshAccessBadges(World world)
        {
            var tracker = TrecsAccessRegistry.GetTracker(world);
            if (tracker == null)
            {
                foreach (var kv in _accessBadges)
                {
                    if (kv.Value != null)
                    {
            ...
            foreach (var kv in _accessBadges)
            {
                ...
                try
                {
                    id = new ComponentId(TypeIdProvider.GetTypeId(type));
                }
                catch (Exception)
                {
                    badge.style.display = DisplayStyle.None;
                ...
                int hash = (readers?.Count ?? 0) | ((writers?.Count ?? 0) << 16);
                if (_lastAccessHashByType.TryGetValue(type, out var prev) && prev == hash)
                {
                    continue;
                }
                _lastAccessHashByType[type] = hash;
                ...
                {
                    badge.text = $"R {readers.Count} · W {writers.Count}";
                    badge.tooltip =
                        "Read by "
                        + (readers.Count == 0 ? "(none)" : string.Join(", ", readers))
                        + "\nWritten by "
        ...
        {
            var name = t.Name;
            var prefix = GetComponentPrefix(t.Assembly);
            if (
