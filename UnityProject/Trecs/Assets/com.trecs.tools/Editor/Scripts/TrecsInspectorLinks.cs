...
namespace Trecs.Tools
{
    ...
    public static class TrecsInspectorLinks
    {
        ...
        public static VisualElement MakeSetLink(World world, SetDef setDef, string text)
        {
            return BuildLinkRow(
                IconForFolder(),
                text,
                onMouseEnter: () => PreviewSetRequested?.Invoke(world, setDef.Id),
                onMouseLeave: () => PreviewClearRequested?.Invoke(),
                onClick: () => SelectSet(world, setDef)
            );
        }
        ...
        public static void SelectSet(World world, SetDef setDef)
        {
            if (world == null || setDef.Id.Id == 0)
            {
                return;
            }
