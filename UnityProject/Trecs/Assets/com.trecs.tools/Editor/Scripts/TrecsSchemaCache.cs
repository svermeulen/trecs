    ...
    public static class TrecsSchemaCache
    {
        ...
        static TrecsSchema Build(World world)
        {
            ...
            foreach (var setDef in info.AllSets)
            {
                ...
                {
                    DebugName = setDef.DebugName ?? $"#{setDef.Id.Id}",
                    Id = setDef.Id.Id,
            ...
            foreach (var setDef in info.AllSets)
            {
                AccumulateTagsFromTagSet(tagsByGuid, setDef.Tags);
            }
            ...
            foreach (var setDef in info.AllSets)
            {
                var setName = setDef.DebugName ?? $"#{setDef.Id.Id}";
                AddTagSetCrossLink(tagsByGuid, setDef.Tags, t => t.SetNames.Add(setName));
            }
