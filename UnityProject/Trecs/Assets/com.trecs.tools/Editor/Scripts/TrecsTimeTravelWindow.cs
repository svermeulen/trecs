...
namespace Trecs.Tools
{
    ...
    public class TrecsTimeTravelWindow : EditorWindow
    {
        ...
        [MenuItem("Svkj/Trecs/Windows/Replay")]
        ...
        void RefreshTick()
        {
            ...
            if (_selectedWorld.IsDisposed)
            {
                ...
                UpdateBadge(GameStateMode.Idle, paused: false, byCapacity: false, recorder: null);
                return;
            }
            ...
            var mode = controller?.CurrentMode ?? GameStateMode.Idle;
            var paused = controller?.IsPaused ?? true;
            var recorder = controller?.AutoRecorder;
            var byCapacity = recorder?.IsPausedByCapacity ?? false;

            UpdateBadge(mode, paused, byCapacity, recorder);
