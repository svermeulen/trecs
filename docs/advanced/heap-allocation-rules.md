...
This page covers when and where you can allocate heap pointers and create entities. The rules exist because Trecs is built around deterministic simulation: for replay and snapshot/restore to work, the same code must produce the same world state on every run.

Pointer mechanics (`SharedPtr<T>`, `UniquePtr<T>`, native variants) are on the [Heap](heap.md) page. Per-role permissions for non-heap concerns (components, structural changes, RNG) are on [Accessor Roles](accessor-roles.md).

## Heap allocation by role

Every `WorldAccessor` carries an [`AccessorRole`](accessor-roles.md) that controls what heap allocation it can perform. Input-system accessors (system-owned, from `[ExecuteIn(SystemPhase.Input)]`) carry `Variable` plus an internal input flag — shown as "Input system" below.
...
Violations throw a `TrecsException` at the allocation site. The input-system error points at the `FrameScoped` variant explicitly, since that's almost always what the caller wanted.

Persistent allocations participate in deterministic ID minting (next section), so only `Fixed` and `Unrestricted` may make them. Frame-scoped allocations exist for input-side transient payloads, so only input-system and `Unrestricted` may make them. `Variable` accessors can do neither.
...
Heap IDs come from a single deterministic RNG shared across init and all fixed systems. If presentation systems could mint IDs they'd either pollute that stream or need a non-deterministic fallback — both break replay. Structural changes follow the same logic: simulation-state templates participate in the snapshot checksum, and presentation systems run at non-deterministic per-render-frame cadence.

`[VariableUpdateOnly]` templates are the exception (cameras, view-only helpers): their groups are render-cadence state, skipped from the checksum, so presentation- and input-phase systems *can* spawn VUO entities. They still can't allocate heap.
...
Every allocation has a `BlobId`. `AllocShared(T blob)` (and friends) mints one by drawing from a deterministic RNG shared across init and all fixed systems, seeded from `WorldSettings.RandomSeed`. If the calling code is deterministic, the IDs are deterministic too, and snapshots and replays work transparently.

The discipline matches fixed-update systems: **init and fixed code must be deterministic.** Conditional allocation based on `UnityEngine.Random`, `DateTime.Now`, filesystem ordering, etc. changes IDs between runs the same way it would desync a fixed-update system. When you can't satisfy that, use explicit `BlobId`s (next section).

## Stable BlobIds — when init isn't deterministic

If your initialization is non-deterministic (filesystem ordering, asset bundle availability, user choices), opt out of auto-ID minting by supplying explicit `BlobId`s:
...
`BlobId` is an opaque `long`. Pick any non-zero value; the identifier must be consistent across runs. Hand-authored IDs are useful for content-pipeline patterns where content is identified by a stable name regardless of load order.
