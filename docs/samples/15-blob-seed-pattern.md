...
**Source:** `com.trecs.core/Samples~/Tutorials/15_BlobSeedPattern/`
...
Entity spawners call `SharedPtr.Acquire<T>(world.Heap, stableId)` — the **lookup-only** path. It finds the existing blob by ID, bumps its refcount, and returns a fresh handle:
...
world
    ...
    .Set(new PaletteRef
    {
        Value = SharedPtr.Acquire<ColorPalette>(world.Heap, paletteId),
        CycleSpeed = 0.3f,
    });
```
...
- **`SharedPtr.Alloc(heap, id, value)` vs `SharedPtr.Acquire(heap, id)`** — seeding (creates the blob) vs lookup (acquires another handle to an already-seeded blob)
